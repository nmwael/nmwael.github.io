...
## Podman does not support GPGCards on windows

create key on windows, import on vm
[releated issue here](https://github.com/containers/podman/issues/23831)

```console
# Generate key
ssh-keygen -t ed25519 -f $env:USERPROFILE\.ssh\podman_key

# Copy public key to server
type $env:USERPROFILE\.ssh\podman_key.pub | ssh root@192.168.245.128 "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys"

# Add Podman connection
C:\Users\****\tools\podman\podman.exe system connection add --default podman-box-work ssh://root@192.168.245.128 --identity $env:USERPROFILE\.ssh\podman_key
